...
def parse_args() -> argparse.Namespace:
    """
    Command line interface for tfgating
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version=f"tf-gating version: v{__version__}",
        help="Print tf-gating version",
    )
    parser.add_argument(
        "--output",
        type=str,
...
def main():
    ...
    device = "/CPU:0" if opt.cpu else "/GPU:0"
    assert opt.cpu or gpus
    ...
    tg = TG(sr=fs, nonstationary=opt.nonstationary)
    with tf.device(device):
        y = tg(tf.constant(x, dtype=tf.float32)).numpy()
